...
======================
Three validation layers for publication-defensible LLM-augmented bibliometrics:

  1. Semantic Gap Audit — quantifies search recall improvement
  2. LLM vs Regex Agreement — Cohen's kappa per concept
  3. Self-Consistency Check — same LLM on same papers twice at temp=0
  4. Human-in-the-Loop Dispute Export — flags disagreements for manual review
...
def semantic_gap_audit(
    ...
    Args:
        ...
        n_sample: Number of excluded papers to sample
    ...
    random.seed(seed)
    sample_indices = random.sample(range(n_excluded_total), n_sample)
    sample = excluded.iloc[sample_indices]

    # Check how many of these the LLM says are relevant
    screening_dict = dict(
        zip(screening_results["doi"].astype(str),
            screening_results["relevant"].astype(bool))
    )
...
def concept_agreement(
    ...
    Computes Cohen's kappa per concept.
    ...
    llm_concepts_df["doi"] = llm_concepts_df["doi"].astype(str)
    merged = merged.merge(llm_concepts_df[["doi", "concepts"]], on="doi", how="left")
    ...
    for concept, pattern in CLINICAL_CONCEPTS.items():
        ...
        # Counts
        regex_count = regex_match.sum()
        llm_count = llm_match.sum()
        both = (regex_match & llm_match).sum()
        regex_only = (regex_match & ~llm_match).sum()
        llm_only = (~regex_match & llm_match).sum()
        neither = (~regex_match & ~llm_match).sum()
    ...
    print(f"\n  Mean Cohen's κ across all concepts: {mean_kappa:.3f} ({kappa_interpretation(mean_kappa)})")
    print(f"\n  Per-concept agreement:")
    print(f"  {'Concept':>30s}  {'Regex':>5}  {'LLM':>5}  {'Both':>5}  {'κ':>6}  {'Interpretation'}")
    print(f"  {'─' * 85}")
    for _, row in results_df.iterrows():
        print(f"  {row['concept']:>30s}  {row['regex_count']:>5}  {row['llm_count']:>5}  "
              f"{row['both']:>5}  {row['kappa']:>6.3f}  {row['interpretation']}")
...
def screening_agreement(
    ...
    common_dois = all_dois & set(llm_decisions.keys())
    ...
    agree_pct = agree / len(common_dois) * 100 if common_dois else 0

    # Disagreement breakdown
    keyword_only = np.sum(keyword_arr & ~llm_arr)  # Keyword says yes, LLM says no
    llm_only = np.sum(~keyword_arr & llm_arr)      # LLM says yes, keyword says no

    print(f"  Papers compared: {len(common_dois):,}")
    print(f"  Agreement: {agree:,} ({agree_pct:.1f}%)")
    print(f"  Disagreement: {disagree:,}")
    print(f"    Keyword ✓ / LLM ✗: {keyword_only:,} (keyword may over-include)")
    print(f"    Keyword ✗ / LLM ✓: {llm_only:,} (keyword may miss these)")
...
# ═══════════════════════════════════════════════════════════════════════
# 4. DISPUTE EXPORT (Human-in-the-Loop)
# ═══════════════════════════════════════════════════════════════════════
...
def export_disputes(
    ...
    for _, row in screening_results.iterrows():
        doi = str(row.get("doi", ""))
        llm_relevant = bool(row.get("relevant", False))
        ...
        if llm_relevant != keyword_relevant:
            disputes.append({
                "doi": doi,
                "title": str(df_raw.loc[df_raw["doi"].astype(str) == doi, "title"].iloc[0])
                    if doi in df_raw["doi"].astype(str).values else "",
    ...
    if len(disputes_df) > 0:
        ...
        print(f"\n  ℹ Edit 'human_override' column (include/exclude) to adjudicate.")
    else:
...
def self_consistency_check(
    ...
    This validates that the LLM is deterministic.
...
def print_audit_table(
    ...
    print(f"{'Relevance Screen':<25} "
          ...
          f"{'κ=' + str(kappa_s) + ', +' + str(llm_only) + ' papers':<25}")
    ...
    print(f"{'Search Recall':<25} "
          f"{'Keyword-based':<25} "
          f"{'LLM-augmented':<25} "
          f"{'Recall gain: ' + f'{recall_gain:.1f}%':<25}")
